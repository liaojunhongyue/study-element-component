...
（3）设置组件的绑定值（value/v-model）、 Radio 的 value 以及Radio的中文描述  
...
```
c. 将 type=radio 的单选按钮的 value 设置为 label 的值
```HTML
<span class="el-radio__input">
  <span class="el-radio__inner"></span>
...
```
c. 设置计算属性 model 作为组件内部 v-model 的绑定变量，并且为其设置 getter 和 setter。之后将 input 的 v-model 设置为 model 变量
```javaScript
computed: {
  model: {
    get() {
...
```
b. 设置计算属性 isDisabled 监听 disabled 的改变
```javaScript
computed: {
  ...
  isDisabled() {
...
```
b. 判断在设置 size 属性之前，是否设置了 border 属性，如果设置了将 size 的样式添加到最外层的 label 上面
```HTML
